...
- **`/commit` 步驟 8 的確認問句**: 由「要提交嗎？」改為固定寫法 `要 commit 嗎？（只寫本地，不 push）`。中文的「提交」同時可讀成「送出去給別人」，2026-09-16 造成過一次混淆——分不清停在本地還是會推到遠端。這一問從頭到尾只涵蓋 `git commit`，push 是 `/deploy` 的事、另一次確認。2.7.4 寫的「批准必須是對『要提交嗎』這一問的回應」隨之改為「要 commit 嗎」。
