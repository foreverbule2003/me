/**
 * GameBoy Shell React Component
 * 
 * Replaces the static HTML shell in index.html, about.html, etc.
 * Handles:
...
(function() {
  ...
  const GameBoyShell = ({ children, activePage = 'home' }) => {
    // --- Audio State ---
    ...
    const handlePowerOn = () => {
      ...
      setBootStep('booting');
      setTimeout(() => setShowBootLogo(true), 100); // Trigger logo animation class if needed

      // 3. Play Sound after slight delay (sync with Nintendo logo drop usually)
        ...
        <div className="gb-bezel">
          ...
          <div className="gb-screen-glass relative">
             ...
             <div className="gb-content" style={{ opacity: contentOpacity, transition: 'opacity 0.5s' }}>
               {/* Pass props to children if needed, or context */}
               {children}
             </div>
          </div>
  ...
  // Export to global
  window.GameBoyShell = GameBoyShell;
