...
(async () => {
  ...
  console.log(
    `[Info] Found CB: ${cbInfo.name}, Underlying: ${stockCode}, ConvPrice: ${convPrice}`,
  );
  ...
  try {
    ...
    console.log(
      `[Step 1] Fetching CB data from TPEx API for ${months.join(", ")}...`,
    );
    ...
    for (const rocMonth of months) {
      ...
      if (consecutiveErrors >= 3) {
        console.error(
          `[Stop] Aborting due to ${consecutiveErrors} consecutive API errors. (Likely blocked)`,
        );
        break;
      }
      ...
      try {
        ...
        if (!json.aaData && !json.tables && !json.reportDate) {
          console.log(
            `    [Debug] Raw Response structure invalid:`,
        ...
        else if (json.tables && json.tables.length > 0)
          rows = json.tables[0].data; // Restore this!
        ...
        if (rows && rows.length > 0) {
          ...
          console.log(
            `    No data for ${rocMonth}. Raw: ${JSON.stringify(json).substring(0, 100)}...`,
          );
        }
      } catch (e) {
    ...
    for (const rocMonth of months) {
      ...
      try {
        ...
        if (json && json.stat === "OK" && json.data) {
          ...
          console.log(
            `    [Debug] TWSE Valid JSON but no data: stat=${json.stat}`,
          );
        }
      } catch (e) {
      ...
      if (!stockFound) {
        try {
          ...
          let tpexJson;
          try {
            tpexJson = JSON.parse(text);
          } catch (e) {
            console.log(
              `    [Debug] TPEx Stock JSON Parse Error: ${e.message}`,
            );
          }

          if (tpexJson && tpexJson.aaData && tpexJson.aaData.length > 0) {
            console.log(
              `    Got ${tpexJson.aaData.length} records (TPEx Stock).`,
            );
    ...
    if (fs.existsSync(outFile)) {
      try {
        ...
        console.warn("[Merge] Failed to read existing JSON, starting fresh.");
      }
    }
    ...
    console.log(
      `  Successfully saved ${finalSorted.length} records to ${outFile}`,
    );
  } catch (e) {
