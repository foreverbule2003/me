    ...
    page.on("response", async (response) => {
      ...
      if (status === 200 && contentType.includes("application/json")) {
        ...
        try {
          ...
          console.log(
            `✅ JSON saved to ${filename} (URL: ${url.substring(0, 100)})`,
          );

          if (data.aaData) {
            fs.writeFileSync(
              "tools/tpex_final_data.json",
    ...
    try {
      const clicked = await page.evaluate(() => {
        ...
        const btn = buttons.find(
          (b) =>
            b.textContent.includes("查詢") ||
            (b.value && b.value.includes("查詢")),
        );
        if (btn) {
          btn.click();
          return "Clicked button by text '查詢'";
        }
        const commonBtn = document.querySelector(
          ".btn-query, .btn-search, #btnQuery",
        );
        if (commonBtn) {
          commonBtn.click();
