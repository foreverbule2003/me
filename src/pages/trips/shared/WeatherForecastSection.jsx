                ...
                <div className="flex overflow-x-auto pb-4 px-4 md:px-0 gap-3 snap-x snap-mandatory no-scrollbar">
                    {forecastData.map((item, idx) => {
                        ...
                        return (
                            <div
                                key={idx}
                                className={`snap-center shrink-0 w-[100px] flex flex-col items-center p-3 rounded-2xl border transition-all ${isToday
                            ...
                            >
                                {/* Day Index & Date */}
                                <div
                                    className={`text-center mb-2 w-full border-b pb-1 ${isToday ? "border-indigo-100" : "border-gray-50"
                                        }`}
                                >
                                    {isToday ? (
                                    ...
                                    <div className="flex items-baseline justify-center gap-1">
                                        <span
                                            className={`text-sm font-black leading-tight ${isToday ? "text-indigo-600" : "text-indigo-900"
                                                }`}
                                        >
                                            {item.date.split("/")[1]}
                                        </span>
                                        <span
                                            className={`text-[10px] font-bold uppercase tracking-wider ${isToday ? "text-indigo-400" : "text-gray-400"
                                                }`}
                                        >
                                            ({item.day})
                                        </span>
                                ...
                                <div className="flex flex-col items-center mb-2 w-full">
                                    ...
                                    <div className="w-full h-1 rounded-full bg-gray-100 mt-1.5 overflow-hidden relative">
                                        <div
                                            className={`absolute top-0 bottom-0 rounded-full opacity-40 ${item.tempHigh >= 15 ? "bg-orange-500" : "bg-blue-500"
                                                }`}
                                            style={{
                                                left: "10%",
                                                right: "10%",
                                ...
                                <div className="mt-auto flex flex-col items-center w-full">
                                    ...
                                    <div className="h-[14px] flex items-center justify-center mb-1">
                                        {item.precip !== "0%" && (
                                            <div
                                                className={`flex items-center gap-0.5 text-[9px] font-bold ${parseInt(item.precip) >= 30
                                                        ? "text-blue-500"
                                                        : "text-gray-300"
                                                    }`}
                                            >
                                    ...
                                    <div className="h-[20px] flex items-center justify-center w-full">
                                        {item.note && (
                                            <div
                                                className={`text-[9px] px-1.5 py-0.5 rounded border truncate max-w-full ${item.warn
