...
const TARGET_URL =
  "http://localhost:5173/me/tools/archive/cb-calculator-standalone.html";
...
async function runTest() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "⚠️ Dropdown not found (maybe auto-searched?), checking loading state...",
      );
    }

    // Step 3: Wait for Chart
    console.log("⏳ Waiting for chart to render...");
    await page.waitForSelector("#premiumChart", {
      visible: true,
      timeout: 10000,
    });

    // Check if chart class contains hidden
    const isHidden = await page.$eval("#premiumChart", (el) =>
    ...
    if (infoText.includes("0 筆")) {
      console.warn(
        "⚠️ Chart shows 0 data points (maybe mock data didn't load?)",
      );
    } else {
